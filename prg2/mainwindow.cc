...
//    ui->output->appendPlainText("Update view:");
    ...
    for (auto& beaconid : beacons)
    {
        ...
        QColor beaconcolor = Qt::magenta;
        if (rgb.r >= 0 && rgb.r < 256 && rgb.g >= 0 && rgb.g < 256 && rgb.b >= 0 && rgb.b < 256)
        {
            beaconcolor.setRgb(rgb.r, rgb.g, rgb.b);
        }
...
//                pen.setWidth(5*fontscale);
    ...
    if (ui->fibres_checkbox->isChecked())
    {
        ...
        for (auto& xy1 : xpoints)
        {
            ...
            QColor xpointcolor = Qt::lightGray;
            ...
            {
                x = 0; y = 0;
                xpointcolor = Qt::green;
            }

            QPen xpointpen(xpointcolor);
            xpointpen.setWidth(0); // Cosmetic pen
            auto dotitem = gscene_->addEllipse(20*x-4*fontscale, -20*y-4*fontscale, 8*fontscale, 8*fontscale,
                                               xpointpen, QBrush(Qt::black));
            dotitem->setZValue(-1);
            dotitem->setFlag(QGraphicsItem::ItemIsSelectable);
            dotitem->setData(0, QVariant::fromValue(XpointInfo{NO_ID, xy1}));
